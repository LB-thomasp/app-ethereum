...
class AddressBookClient(TlvSerializable):
    ...
    def send_async_raw(self, p1: AddressBookSubCommand, payload: bytes):
        ...
        if p1 not in (AddressBookSubCommand.SUB_CMD_EDIT_IDENTIFIER,
                      ...
                      AddressBookSubCommand.SUB_CMD_PROVIDE_LEDGER_ACCOUNT_CONTACT):
    ...
    def prepare_edit_ledger_account(self,
        ...
        """
        assert old_account_name and len(old_account_name) <= CONTACT_NAME_MAX_LENGTH, \
            f"Old account name required (max {CONTACT_NAME_MAX_LENGTH} chars)"
        assert new_account_name and len(new_account_name) <= CONTACT_NAME_MAX_LENGTH, \
    ...
    def prepare_register_ledger_account(self,
        ...
        """
        assert contact_name and len(contact_name) <= CONTACT_NAME_MAX_LENGTH, \
    ...
    def prepare_register_identity(self,
                                  ...
                                  hmac_proof: Optional[bytes] = None) -> bytes:
        """
        ...
        assert derivation_path, "Derivation path is required"
        assert chain_id > 0, "Chain ID must be greater than 0"
        assert (group_handle is None) == (hmac_proof is None), \
        ...
        if hmac_proof is not None:
            assert len(hmac_proof) == HMAC_PROOF_LENGTH, \
        ...
        payload += self.serialize_field(FieldTag.ACCOUNT_IDENTIFIER, address)
        payload += self.serialize_field(FieldTag.DERIVATION_PATH, path_bytes)
        payload += self.serialize_field(FieldTag.CHAIN_ID, chain_id)
        payload += self.serialize_field(FieldTag.BLOCKCHAIN_FAMILY, 1)  # Ethereum
        if group_handle is not None:
    ...
    def prepare_edit_identifier(self,
        ...
        assert derivation_path, "Derivation path is required"
        assert chain_id > 0, "Chain ID must be greater than 0"
        assert len(hmac_proof) == HMAC_PROOF_LENGTH, f"HMAC_PROOF must be {HMAC_PROOF_LENGTH} bytes"
        assert len(hmac_rest) == HMAC_PROOF_LENGTH, f"HMAC_REST proof must be {HMAC_PROOF_LENGTH} bytes"
        ...
        payload += self.serialize_field(FieldTag.CONTACT_NAME, contact_name.encode('utf-8'))
        payload += self.serialize_field(FieldTag.SCOPE, scope.encode('utf-8'))
        payload += self.serialize_field(FieldTag.ACCOUNT_IDENTIFIER, new_address)
    ...
    def prepare_edit_contact_name(self,
        ...
        Returns:
            Complete TLV payload bytes.
        """
        assert old_contact_name and len(old_contact_name) <= CONTACT_NAME_MAX_LENGTH, \
            f"Previous contact name required (max {CONTACT_NAME_MAX_LENGTH} chars)"
        assert new_contact_name and len(new_contact_name) <= CONTACT_NAME_MAX_LENGTH, \
    ...
    def prepare_edit_scope(self,
        ...
        assert contact_name and len(contact_name) <= CONTACT_NAME_MAX_LENGTH, \
            f"Contact name required (max {CONTACT_NAME_MAX_LENGTH} chars)"
        assert len(group_handle) == GROUP_HANDLE_LENGTH, f"group_handle must be {GROUP_HANDLE_LENGTH} bytes"
        assert old_scope and len(old_scope) <= SCOPE_MAX_LENGTH, \
            f"Previous scope required (max {SCOPE_MAX_LENGTH} chars)"
        assert new_scope and len(new_scope) <= SCOPE_MAX_LENGTH, \
    ...
    def prepare_provide_contact(self,
        ...
        assert contact_name and len(contact_name) <= CONTACT_NAME_MAX_LENGTH, \
            f"Contact name required (max {CONTACT_NAME_MAX_LENGTH} chars)"
        assert scope and len(scope) <= SCOPE_MAX_LENGTH, \
    ...
    def prepare_provide_ledger_account_contact(self,
                                               hmac_proof: bytes,
                                               contact_name: str = DEFAULT_ACCOUNT_NAME,
                                               derivation_path: str = DEFAULT_BIP32_PATH,
                                               chain_id: int = DEFAULT_CHAIN_ID) -> bytes:
        """
        Prepare TLV payload for the Provide Ledger Account Contact command (P1=0x21).
        ...
        hmac_proof (the HMAC Proof of Registration from register_ledger_account).
        ...
        payload: bytes = self.serialize_field(FieldTag.STRUCT_TYPE,
                                              AddressBookResponseType.TYPE_PROVIDE_LEDGER_ACCOUNT_CONTACT)
        payload += self.serialize_field(FieldTag.STRUCT_VERSION, 1)
        payload += self.serialize_field(FieldTag.CONTACT_NAME, contact_name.encode('utf-8'))
        payload += self.serialize_field(FieldTag.DERIVATION_PATH, path_bytes)
        payload += self.serialize_field(FieldTag.CHAIN_ID, chain_id)
