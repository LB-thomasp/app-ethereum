 ...
 * UI is displayed. Derives the Ethereum address from the BIP32 path and stores
 * it in @p params->address so that on_edit_ledger_account_applied() can locate
 * the cache entry without re-deriving.
 *
 * @param[in,out] params Parsed edit data; address field is filled on success
 * @return true if derivation succeeded, false to reject
 */
